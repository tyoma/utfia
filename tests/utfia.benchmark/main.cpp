...
namespace
{
	const auto c_n_iterations = 300000;
	...
	{
		unsigned l = 0;
		auto pbegin = utf8_string.c_str();
		auto pend = utf8_string.c_str() + utf8_string.length();

		for (auto j = pbegin; j != pend; utfia::utf8::next(j, pend))
			l++;
		return l;
	}
...
int main()
{
	...
	auto text = c_roll_the_dice + c_choven + c_chinese + c_chinese + c_chinese;
//	auto text = c_roll_the_dice;
//	auto text = c_choven;
//	auto text = c_chinese;
	auto length = utf8_length(text);

	start_time = clock();
	for (auto i = c_n_iterations; i; --i)
		sum_ += utf8_length(text);
	auto taken = 1.0 * (clock() - start_time) / CLOCKS_PER_SEC / length / c_n_iterations;
